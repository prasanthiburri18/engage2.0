...
public class EventsController {
	...
	public @ResponseBody JsonMessage addEvent(@RequestBody final Events events) {
		...
		try {
			...
			Set<ConstraintViolation<Events>> violations = validator
					.validate(events);
			...
			Boolean isPathway = _eventsDao.getByEventName(
					events.getEventName(), events.getPathwayId());
			if (isPathway == true) {
	...
	public @ResponseBody JsonMessage listEvents(
		...
		try {
			...
			List<Events> events = _eventsDao.getAll(Long.parseLong(json
					.get("pathwayId")));
	...
	/**
	 * List Event Method Listing Events For Pathway
	 * 
	 * @Inutparam JsonObject
	 * @return JsonObject
	 */
	...
	public @ResponseBody JsonMessage listEventsbyPathwayId(
		...
		try {
			...
			List<Object> events = _eventsDao.getEventnamesbyPathwayId(Long
					.parseLong(json.get("pathwayId")));
